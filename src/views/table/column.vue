  ...
  <el-table
    ...
    style="width: 100%">
          ...
          <el-table-column prop="state" label="状态" width="50">
            ...
            <span v-if="scope.row.state==1" style="color:green;">已通过</span>
    ...
    <el-table-column
      ...
      <template slot-scope="scope">
        <el-button @click="handleEdit(scope.row.id)" type="text" size="small">专栏详情</el-button>
      </template>
    </el-table-column>
  </el-table>
      <el-pagination
  ...
  methods: {
    ...
    handleSave() {
      if (this.id !== '') {
        ...
        columnApi.save(this.pojo).then(response => {
          this.$message({
            message: response.message,
            type: (response.flag ? 'success' : 'error')
          })
